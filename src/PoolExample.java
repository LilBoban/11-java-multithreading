...
public class PoolExample {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        for (int i = 0; i < 30; i++) {
            ...
            // Если очередь задач переполнена, ждём освобождения места
            while (executor.getQueue().remainingCapacity() == 0) {
                Thread.sleep(10); // Ждём 50 миллисекунд перед повторной проверкой
            }
        ...
        executor.shutdown(); // Говорим пулу потоков, что больше задач не будет
        executor.awaitTermination(1, TimeUnit.MINUTES); // Ждём завершения всех задач (до часа времени)
    }
}
